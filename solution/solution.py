...
def compare_models(prompt: str) -> dict:
    ...
    # Approximate: 0.75 words ≈ 1 token
    token_count = len(gpt4o_response.split()) / 0.75
    gpt4o_cost = (token_count / 1000) * COST_PER_1K_OUTPUT_TOKENS["gpt-4o"]
...
def streaming_chatbot() -> None:
    ...
    print("Streaming Chatbot (type 'quit' or 'exit' to stop)")
    print("=" * 50)
    ...
    while True:
        ...
        # Keep only last 3 turns (3 user + 3 assistant = 6 messages max)
        history = history[-6:]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        ...
        table += f"{prompt:<40} | {gpt4o_resp:<40} | {mini_resp:<40} | {gpt4o_lat:<15} | {mini_lat:<15}\n"
